    ...
    let pushed = {
        let uid_i64 = {
            state
                .chat_states
                .get(chat_id)
                .and_then(|cs| cs.ask_session.as_ref().map(|s| s.user_id))
                .unwrap_or(0)
        };
        let mut entry = state.chat_states.entry(chat_id.to_string()).or_default();
        let session = entry.ask_session.get_or_insert_with(|| {
            let mut s = AskSession::new(uid_i64, repo_path_opt.clone(), git_opt.clone());
...
pub async fn handle_ask(
    ...
    let all_repos: Vec<(String, std::path::PathBuf)> =
        accessible_repos(&state, uid_i64, &is_authorized_for)
            .into_iter()
            .map(|(k, p, _)| (k, p))
            .collect();
    ...
    if all_repos.is_empty() {
        ...
        if question.is_empty() {
            ...
            {
                let mut entry = state.chat_states.entry(chat_id.to_string()).or_default();
                entry.ask_session = Some(AskSession::new(uid_i64, None, None));
            }
    ...
    if all_repos.len() == 1 {
        ...
        let session = if let Some(mg) = main_git {
            ...
            AskSession::new(uid_i64, Some(repo_path.clone()), None)
        };
...
pub async fn handle_ask_session_callback(
    ...
    if action_data.starts_with("ask:repo:") {
        ...
        let all_repos = accessible_repos(&state, uid_i64, &is_authorized_for);
