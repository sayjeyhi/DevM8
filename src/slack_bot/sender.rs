...
/// Maximum characters per Slack message (Block Kit mrkdwn section limit).
...
impl SlackSender {
    ...
    /// POST to `chat.postMessage`; returns the message timestamp (`ts`).
    ...
    /// Build a Block Kit `blocks` array from a keyboard.
...
impl ChannelSender for SlackSender {
    async fn send(&self, chat_id: &str, text: &str) -> anyhow::Result<SentMessageRef> {
        ...
        let ts = self
            .post_message_raw(chat_id, &text, Some(blocks))
            .await?;
    ...
    async fn remove_keyboard(&self, msg_ref: &SentMessageRef) -> anyhow::Result<()> {
        // Re-fetch current text is expensive; callers should pass the current text
        // when removing keyboards. Use a no-op block (section only, no actions).
        // We update with only a section block to strip the action buttons.
        let placeholder = "(keyboard removed)";
        let blocks = json!([{
            "type": "section",
            "text": { "type": "mrkdwn", "text": placeholder }
        }]);
        self.update_message_raw(
            &msg_ref.chat_id,
            &msg_ref.message_id,
            placeholder,
            Some(blocks),
    ...
    /// Slack does not have a conventional "typing…" indicator for bots.
    /// Return a no-op handle — the caller will abort it when done, which is fine.
    ...
    fn system_context_prefix(&self) -> &'static str {
        "You are replying through the DevM8 Slack bot."
    }
}
